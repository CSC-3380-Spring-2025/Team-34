"""Home Page rendering for the LSU Datastore Dashboard.

This module provides the render_home_page function to display a Streamlit page for
data management, visualization, search, live logging, and system performance
monitoring. It relies on environment variables (e.g., DATABASE_NAME, LOG_DIR,
SENDGRID_API_KEY, FROM_EMAIL) loaded from a .env file (ignored by .gitignore) and
generates files (CSV, Parquet) tracked by Git LFS (per .gitattributes).
"""
...
def render_home_page() -> None:
    ...
    Displays an overview of the LSU Datastore, allows logged-in users to upload,
    edit, and delete datasets, provides data visualization, search, live logging,
    and system performance metrics. Logs user actions to LOG_DIR/LOG_FILE.

    Raises:
        sqlite3.Error: If a database query fails during file operations or search.
        FileNotFoundError: If image files (lsu_data_icon.png, lsu_logo.png) are missing.
        sendgrid.SendGridException: If email sharing fails.
    """
    ...
    if st.session_state.logged_in:
        ...
        if manage_files:
            manage_file_options: Dict[int, str] = {
                file_id: filename
                for file_id, filename, size, file_type, created_at in manage_files
            }
            manage_file_id: int = st.selectbox(
            ...
            if manage_file_id:
                manage_df: pd.DataFrame = cached_get_csv_preview(manage_file_id)
                if not manage_df.empty:
                    st.write(f"**Preview of {manage_file_options[manage_file_id]}:**")
                    st.dataframe(manage_df)

                    st.subheader("Edit Data")
                    edited_df: pd.DataFrame = st.data_editor(manage_df)
                    if st.button("Save Changes"):
                        ...
                        logger.info(
                            ...
                            extra={
                                "username": st.session_state.username or "Anonymous",
                                "action": "edit_dataset",
                                "details": f"Edited: {manage_file_options[manage_file_id]}",
                            },
                        )

                    csv_data: bytes = manage_df.to_csv(index=False).encode("utf-8")
                    json_data: str = manage_df.to_json(orient="records")
                    output: io.BytesIO = io.BytesIO()
                    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
                        manage_df.to_excel(writer, index=False)
                    excel_data: bytes = output.getvalue()

                    csv_col, json_col, excel_col = st.columns(3)
                    with csv_col:
                        st.download_button(
                            label="Download CSV",
                            data=csv_data,
                    ...
                    if st.button("Delete This Dataset"):
                        ...
                        logger.info(
                            ...
                            extra={
                                ...
                                "details": f"Deleted: {manage_file_options[manage_file_id]}",
                            },
                        )
                        st.rerun()
                else:
    ...
    major_col, category_col, _ = st.columns([1, 1, 3])
    with major_col:
        majors: list[str] = [
        ...
        selected_major: str = st.selectbox(
            "Filter by Major:",
            majors,
    ...
    with category_col:
        categories: list[str] = ["jobs", "courses", "research", "lsu"]
    ...
    st.markdown(
        """
        The LSU Datastore is a tool for managing and analyzing datasets at LSU,
        including research projects, jobs, and courses. The Datastore provides a
        continuously updated platform for accessing data, performing searches, and
        sharing insights. Details of our work are provided in the LSU Data Science
        Repository. We hope that researchers will use the LSU Datastore to gain
        insights into academic and professional opportunities.
        """
    )
    ...
    formatted_date: str = selected_date.strftime("%Y-%m-%d")
    ...
    if files:
        ...
        if file_options:
            dataset_col, logo_col = st.columns([3, 1])
            with dataset_col:
    ...
    if files and file_options and selected_file_id and not df.empty:
        ...
        global_search: str = st.text_input("Search across all datasets:", key="global_search_home")
        if global_search:
            results = search_csv_data(global_search)
            if results:
                st.dataframe(
                    pd.DataFrame(
                        results, columns=["File ID", "Row", "Column", "Value"]
                    ),
                    hide_index=True,
                )
            else:
    ...
    if st.session_state.logged_in:
        ...
        log_files: list[str] = [
            ...
            if f.startswith("live_feed_log") and f.endswith(".csv")
        ]
        if log_files:
            ...
            if selected_log:
                log_path: str = os.path.join(log_dir, selected_log)
                try:
                    log_df: pd.DataFrame = pd.read_csv(log_path)
                    st.write(f"**Log: {selected_log}**")
                    st.dataframe(log_df, hide_index=True)

                    log_csv: bytes = log_df.to_csv(index=False).encode("utf-8")
                    st.download_button(
                    ...
                    logger.error(
                        ...
                        extra={
                            "username": st.session_state.username or "Anonymous",
                            "action": "read_log",
                            "details": f"Error: {e}, Log: {selected_log}",
                        },
                    )
        else:
