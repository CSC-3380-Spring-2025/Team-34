...
with st.container():
    ...
    page = st.selectbox(
        "Navigate to:",
        ["Home", "Browse CSV Files"],
        key="page_select",
        index=["Home", "Browse CSV Files"].index(st.session_state.page)
    )
    st.session_state.page = page

    if page == "Browse CSV Files":
        st.markdown('<div class="main">', unsafe_allow_html=True)
        st.header("Browse CSV Files")
                ...
                if not df.empty:
                    ...
                    with col_dl1:
                        ...
                        if st.download_button(
                            ...
                            mime="text/csv",
                            key="download_csv_browse"
                        ):
                            logger.info(
                                "CSV Downloaded",
                                extra={
                                    "username": st.session_state.username or "Anonymous",
                                    "action": "download_csv_browse",
                    ...
                    with col_dl2:
                        ...
                        if st.download_button(
                            ...
                            key="download_parquet_browse"
                        ):
                            logger.info(
                                "Parquet Downloaded",
                                extra={
                                    "username": st.session_state.username or "Anonymous",
                                    "action": "download_parquet_browse",
    ...
    else:  # Home page (original content)
        st.markdown('<div class="main">', unsafe_allow_html=True)
        ...
        try:
            st.image(os.path.join(base_dir, "lsu_data_icon.png"), caption="LSU Dat FELstore Visualization", use_container_width=True, output_format="PNG")
        ...
        st.markdown("""
            ...
            - **Browse CSV Files**: View all CSV files stored in the LSU Datastore (no login required).
                                ...
                                if email_input:
                                    ...
                                    if not re.match(email_regex, email_input):
                                        ...
                                        try:
                                            ...
                                            # Use environment variable for sender email or fallback
                                            sender_email = os.environ.get('SENDER_EMAIL', 'bdav213@lsu.edu')
                                            ...
                                            if response.status_code == 202:
                                                ...
                                                logger.info(
                                                    ...
                                                    extra={
                                                        "username": st.session_state.username or "Anonymous",
                                                        "action": "email_share",
                                                        "details": f"Failed, Status code: {response.status_code}, Response: {response.body}, Dataset: {file_options[selected_file_id]}"
                                                    }
                                                )
