...
from src.datastore.database import save_csv_to_database, init_db
...
def fetch_jobs_data(major: str) -> pd.DataFrame:
    ...
    Args:
        major: The major to search for (e.g., 'software engineering').

    Returns:
        A DataFrame containing job data (title, company, location, url, posted_date).

    Raises:
        requests.HTTPError: If the API request fails due to an HTTP error.
        ValueError: If the response cannot be parsed as JSON.
    """
    # return pd.DataFrame()  # Use this to skip API calls during testing
    try:
...
def fetch_courses_data(major: str) -> pd.DataFrame:
    ...
    Args:
        major: The major to search for (e.g., 'data science').

    Returns:
        A DataFrame containing course data (title, instructor, price, url, last_updated_date).
        ...
        course_data = [
            {
                ...
                "instructor": ", ".join(
                    [instr.get("display_name", "") for instr in course.get("instructors", [])]
                ),
...
def fetch_research_data(major: str) -> pd.DataFrame:
    ...
    Args:
        major: The major to search for (e.g., 'cybersecurity').

    Returns:
        A DataFrame containing research data (title, authors, publication_date, url).
    ...
    try:
        ...
        response = requests.get(
            "https://api.core.ac.uk/v3/search/works", headers=headers, params=params
        )
        response.raise_for_status()
        data = response.json()
        research_data = [
...
def fetch_lsu_course_data() -> pd.DataFrame:
    ...
    Returns:
        A DataFrame containing LSU course data with columns such as course code,
...
def save_to_database(dataframe: pd.DataFrame, filename: str, user_id: int = 1) -> None:
    ...
    Args:
        ...
        user_id: User ID for database storage. Defaults to 1.
    ...
    save_csv_to_database(filename, file_content, len(file_content), "csv", user_id)
